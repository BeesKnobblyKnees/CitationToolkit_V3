...
def check_routing():
    ...
    pages = ["citation_repair","library_tools","bibliography_audit",
             "finalise","figure_inventory","figure_extractor"]
...
def check_app_labels():
    # Every page file should have an app-label div or a page title
    for fn in ["page_citation_repair.py","page_library_tools.py",
               "page_bibliography_audit.py","page_finalise.py",
               "page_figure_inventory.py","page_figure_extractor.py"]:
        path = BASE / fn
        if not path.exists():
            raise AssertionError(f"{fn} not found")
        content = path.read_text()
        if "app-label" not in content and "st.markdown" not in content:
