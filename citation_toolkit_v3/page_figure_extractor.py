...
def pdf_scan(pdf_bytes, dpi=200):
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    total = len(doc); doc.close()
    figures = []; seen = set()
    bar = st.progress(0, text="Scanning pages…")
    for pn in range(total):
        bar.progress((pn+1)/total, text=f"Scanning page {pn+1}/{total}…")
        doc  = fitz.open(stream=pdf_bytes, filetype="pdf")
...
if uploaded:
    ...
    if src_ext == '.pdf':
        ...
        st.caption(f"{uploaded.name} — {len(doc.pages)} pages")
        doc.close()
    else:
    ...
    if st.button("Scan for figures", type="primary", key="fe_scan"):
        with st.spinner("Scanning…"):
            figs = pdf_scan(src_bytes, dpi=scan_dpi) if src_ext=='.pdf' else docx_scan(src_bytes)
